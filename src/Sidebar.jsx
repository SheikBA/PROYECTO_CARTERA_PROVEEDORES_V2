    ...
    return (
        <aside className={`${sidebarCollapsed ? 'w-20' : 'w-64'} bg-surface border-r border-slate-200 transition-all duration-300 flex flex-col z-20 shadow-lg relative`}>
            <div className="h-16 flex items-center justify-center border-b border-slate-100 relative shrink-0">
                <div className="h-8 w-8 bg-gradient-to-br from-[#0082a6] to-[#195655] rounded-lg flex items-center justify-center text-white shadow-md">
                    <Landmark size={18} />
                </div>
                {!sidebarCollapsed && (
                    <span className="font-black ml-2 text-slate-800 text-xs tracking-tighter uppercase leading-none">
                        PAGOS | Tesorería H2H
            ...
            <div className="p-4 border-t border-slate-100 shrink-0">
                <button
                    onClick={onLogout}
                    className={`w-full flex items-center ${sidebarCollapsed ? 'justify-center' : ''} gap-3 px-3 py-2 text-red-500 bg-red-50/50 hover:bg-red-500 hover:text-white rounded-lg transition-all duration-300 border border-red-100`}
